...
const handler = async (event) => {
  ...
  const senderName = `c_fit,l_text:Roboto_32:@${encodeURI(
    cardData.senderName
  )},g_south_west,w_340,x_780,y_65`;
  const senderAvatar = `l_fetch:${encodeURL(
    cardData.senderAvatar
  )},r_max,w_70,g_south_west,x_700,y_48`;
  const recipientName = `c_fit,l_text:Roboto_32:@${encodeURI(
    cardData.recipientName
  )},g_south_west,w_340,x_155,y_65`;
  const recipientAvatar = `l_fetch:${encodeURL(
